  ...
  const handleSubmit = async () => {
    ...
    if (!turnstileToken) {
      setStatusModal({
        ...
        message: "Mohon centang kotak verifikasi keamanan (Turnstile) terlebih dahulu.",
        onAction: () => setStatusModal((prev) => ({ ...prev, isOpen: false })),
      });
      return;
    }
    ...
    try {
      // 1. Upload file wajib secara paralel
      ...
      // 3. Mapping data untuk database payload
      const dbPayload = {
          ...
          >
            <motion.div variants={itemVariants}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {["Creative Dance Competition"].map((cat) => (
                  <button
                    key={cat}
                    type="button"
                    onClick={() =>
            ...
            <motion.div variants={itemVariants}>
              ...
              <div className="grid grid-cols-2 gap-4">
                {["Early Bird", "Normal Price"].map((b) => {
                  const isDisabled =
                    b === "Early Bird" ? !isEarlyBirdActive : !isNormalPriceActive;
